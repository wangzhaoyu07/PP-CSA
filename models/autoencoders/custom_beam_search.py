...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        #invalid_idx = list(range(4,50265))
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                    ...
                    if input_ids.shape[-1]==2:
                        if False in list((input_ids[batch_beam_idx,:]==torch.tensor([2,0],device=input_ids.device)).cpu()):
                            print()
                            pass
                    if input_ids.shape[-1]>2 and (tokenizer.decode(torch.cat([input_ids[batch_beam_idx,:], next_token.unsqueeze(-1)], dim=-1), skip_special_tokens=True)\
                        not in gt_graph): 
                        continue
                    '''if 4<=next_token<=50264:
                        print("here")'''
                ...
                if beam_idx == self.group_size:
                    break
            # TODO
            while beam_idx < self.group_size:
                # TODO:
                next_beam_scores[batch_idx, beam_idx] = -1e9 #next_beam_scores[batch_idx, 0]
                next_beam_tokens[batch_idx, beam_idx] = next_beam_tokens[batch_idx, 0]
                next_beam_indices[batch_idx, beam_idx] = next_beam_indices[batch_idx, 0]
                beam_idx += 1
            if beam_idx < self.group_size:
                raise ValueError(
    ...
    def process_new(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                    # if beam_token does not belong to top num_beams tokens, it should not be added
                    is_beam_token_worse_than_top_num_beams = valid_len >= self.group_size
                    if is_beam_token_worse_than_top_num_beams:
                        continue
                    if beam_indices is not None:
                        beam_index = beam_indices[batch_beam_idx]
